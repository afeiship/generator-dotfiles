...
describe('module:prettier', () => {
  test('creates prettier files: ideable-true', async () => {
    await helpers.run(GEN_PATH).withOptions({ ideable: true });
    // normal files
    assert.file(['.idea/prettier.xml', '.prettierrc', '.prettierignore']);
  });

  test('creates prettier files: ideable-false', async () => {
    // no .idea/prettier.xml
    await helpers.run(GEN_PATH).withOptions({ ideable: false });
